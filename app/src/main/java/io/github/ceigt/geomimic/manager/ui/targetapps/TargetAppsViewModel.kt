package io.github.ceigt.geomimic.manager.ui.targetapps
...
import io.github.ceigt.geomimic.data.repository.PreferencesRepository
...
class TargetAppsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun cycleCoordinateSystem(packageName: String) {
        val current = _uiState.value.appCoordinateSystems[packageName] ?: CoordinateSystem.WGS84
        viewModelScope.launch {
            preferencesRepository.saveAppCoordinateSystem(packageName, current.next())
        }
    }

    private fun observeCoordinateSystems() {
        viewModelScope.launch {
            preferencesRepository.getAppCoordinateSystemsFlow().collectLatest { systems ->
                _uiState.update { state ->
                    state.copy(
                        appCoordinateSystems = systems,
                        apps = state.apps.map { app ->
                            app.copy(coordinateSystem = systems[app.packageName] ?: CoordinateSystem.WGS84)
                        }
                    )
                }
            }
                ...
                state.copy(
                    apps = appsWithSystemFramework.map {
                        it.copy(
                            ...
                            coordinateSystem = state.appCoordinateSystems[it.packageName]
