...
Manifest 仅导出启动 Activity。Mock Provider 前台服务不导出；可选外部控制 Receiver 默认禁用，只有用户在设置中主动打开后才能接收本机广播。地图功能会连接百度地图服务；未接受说明或未打开地图时不会加载地图页面。
